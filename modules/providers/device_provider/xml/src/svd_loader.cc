...
// device_() below (generated by xsdcxx from CMSIS_SVD.xsd) parses via
// xsd::cxx::xml::dom::parse<char>(), which needs Xerces-C's global platform
// state set up first - without it, DOMImplementationRegistry::
// getDOMImplementation() null-derefs inside XMLMutexLock. Xerces-C requires
// exactly one Initialize() per process (no balancing Terminate() here - the
// process exiting reclaims it, and calling Terminate() while another thread
// could still be mid-parse would just trade one hazard for another), so a
// call_once-guarded call on first use covers every caller regardless of
// which thread gets there first.
void EnsureXercesInitialized() {
    static std::once_flag once;
    std::call_once(once, [] { xercesc::XMLPlatformUtils::Initialize(); });
}
