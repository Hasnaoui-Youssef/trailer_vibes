...
StepInTargetsRequestHandler::Run(const StepInTargetsArguments &args) const {
  dap.Context().Execution().step_in_targets.clear();
  const lldb::SBFrame frame = dap.Context().GetLLDBFrame(args.frameId);
  if (!frame.IsValid())
  ...
  for (size_t i = 0; i < num_insts; ++i) {
    ...
    if (flow_kind == lldb::eInstructionControlFlowKindCall) {
      ...
      dap.Context().Execution().step_in_targets.try_emplace(inst_addr, step_in_target_name);
