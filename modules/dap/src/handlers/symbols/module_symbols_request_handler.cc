...
ModuleSymbolsRequestHandler::Run(const ModuleSymbolsArguments &args) const {
  ...
  for (size_t i = start_index; i < end_index; ++i) {
    ...
    // dap::protocol::SymbolType mirrors lldb::SymbolType value-for-value
    // (dap_protocol never depends on LLDB - see dap_types.hpp) so this cast
    // is exact.
    dap_symbol.type = static_cast<SymbolType>(symbol.GetType());
