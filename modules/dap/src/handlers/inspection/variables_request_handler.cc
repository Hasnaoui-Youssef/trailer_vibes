...
VariablesRequestHandler::Run(const VariablesArguments &arguments) const {
  ...
  std::optional<ScopeData> scope_data = dap.Context().Data().variables.GetTopLevelScope(var_ref);
  if (scope_data) {
    ...
    if (scope_data && scope_data->kind == eScopeKindLocals) {
      ...
      if (stop_return_value.IsValid() &&
        ...
        if (stop_return_value.MightHaveChildren() ||
          ...
          return_var_ref = dap.Context().Data().variables.InsertVariable(stop_return_value,
                                                        /*is_permanent=*/false);
        }
        variables.emplace_back(core::CreateVariable(
            renamed_return_value, return_var_ref, hex,
            dap.Context().Session().configuration.enableAutoVariableSummaries,
            dap.Context().Session().configuration.enableSyntheticChildDebugging, false));
      }
    }
    ...
    for (auto i = start_idx; i < end_idx; ++i) {
      ...
      variables.emplace_back(core::CreateVariable(
          ...
          dap.Context().Session().configuration.enableAutoVariableSummaries,
          dap.Context().Session().configuration.enableSyntheticChildDebugging,
    ...
    if (variable.IsValid()) {
      const bool is_permanent =
          dap.Context().Data().variables.IsPermanentVariableReference(var_ref);
      auto addChild = [&](lldb::SBValue child,
                          std::optional<llvm::StringRef> custom_name = {}) {
        if (!child.IsValid())
          return;
        const int64_t child_var_ref =
            dap.Context().Data().variables.InsertVariable(child, is_permanent);
        variables.emplace_back(
            core::CreateVariable(child, child_var_ref, hex,
                           dap.Context().Session().configuration.enableAutoVariableSummaries,
                           dap.Context().Session().configuration.enableSyntheticChildDebugging,
      ...
      if (dap.Context().Session().configuration.enableSyntheticChildDebugging &&
