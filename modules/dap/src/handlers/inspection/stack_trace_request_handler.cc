...
// `using namespace dap;` brings both DebugService/StackTraceRequestHandler
// and (via dap::protocol being a nested namespace, not a sibling) enough
// reach for the out-of-line `StackTraceRequestHandler::Run` definition
// below to resolve unqualified - this file's free functions live at file
// scope, never inside `namespace dap {}` itself.
using namespace dap;
...
static StackFrame CreateStackFrame(DebugService &dap, lldb::SBFrame &frame,
  ...
  if (stack_frame.name.empty()) {
    ...
    stack_frame.name = core::GetLoadAddressString(frame.GetPC());
  }
  ...
  std::optional<protocol::Source> source = dap.Context().ResolveSource(frame);
  if (source && !core::IsAssemblySource(*source)) {
...
static bool FillStackFrames(DebugService &dap, lldb::SBThread &thread,
...
StackTraceRequestHandler::Run(const protocol::StackTraceArguments &args) const {
  ...
  bool include_all = dap.Context().Session().configuration.displayExtendedBacktrace;
