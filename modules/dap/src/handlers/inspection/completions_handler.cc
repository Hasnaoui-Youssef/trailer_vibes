...
CompletionsRequestHandler::Run(const CompletionsArguments &args) const {
  ...
  const StringRef escape_prefix = dap.Context().Session().configuration.commandEscapePrefix;
  const bool had_escape_prefix = StringRef(text).starts_with(escape_prefix);
  const ReplMode repl_mode = dap.Context().Data().DetectReplMode(frame, text, true);
