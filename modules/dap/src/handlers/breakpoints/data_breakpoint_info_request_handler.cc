...
static bool IsRW(DebugService &dap, lldb::addr_t load_addr) {
...
DataBreakpointInfoRequestHandler::Run(
  ...
  lldb::SBValue variable = dap.Context().Data().variables.FindVariable(
  ...
  } else if (lldb::SBFrame frame = dap.Context().GetLLDBFrame(args.frameId);
