...
public:
  explicit BreakpointBase(DebugService &d) : m_dap(d) {}
  BreakpointBase(DebugService &d, const std::optional<std::string> &condition,
  ...
  /// labels or categories. All breakpoints that are set through DebugService get sent
  /// through the various DebugService set*Breakpoint packets, and these breakpoints will
  ...
  /// DebugService breakpoint. We can later check a lldb::SBBreakpoint object that comes
  ...
  /// breakpoint in one of the DebugService breakpoints that we should report changes
  /// for.
  static constexpr const char *kDAPBreakpointLabel = "dap";

protected:
  /// Associated DebugService session.
  DebugService &m_dap;
