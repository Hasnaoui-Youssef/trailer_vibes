...
//   }]
// }
llvm::json::Value CreateThreadStopped(DebugService &dap, lldb::SBThread &thread,
                                      uint32_t stop_id) {
  ...
  case lldb::eStopReasonBreakpoint: {
    ...
    if (exc_bp) {
      ...
      // Instruction breakpoints aren't forked yet (InstructionBreakpoint.*,
      // not needed by setInstructionBreakpoints, which isn't registered in
      // this pass) - always reports plain "breakpoint" for now.
...
llvm::json::Object CreateRunInTerminalReverseRequest(
  ...
  // DebugService::debug_adapter_path isn't ported (it's specific to the
  // RunInTerminal flow, deferred - see request_handler.cc); this function
  // is dead code until that lands. Placeholder keeps it compiling.
  std::vector<std::string> req_args = {"trailer-dap", "--comm-file", comm_file.str()};
