...
// `using namespace dap;` (not just its debug_service/protocol children) is
// needed here: this file's free functions live at file scope, not nested
// inside `namespace dap::debug_service {}`, so bare `protocol::X` needs
// `protocol` itself reachable as a name - which only importing dap's own
// members (not debug_service's) provides. See project-dap-layer-fork-
// strategy memory: upstream's single flat `lldb_dap` namespace (holding
// both DAP and its own nested `protocol`) provided this for free; splitting
// it into dap::debug_service/dap::protocol as siblings didn't carry over.
using namespace dap;
