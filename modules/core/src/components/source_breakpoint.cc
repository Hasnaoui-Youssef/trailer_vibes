...
SourceBreakpoint::SourceBreakpoint(DebugContext &context,
...
llvm::Error SourceBreakpoint::CreateAssemblyBreakpointWithSourceReference(
  ...
  std::optional<lldb::addr_t> raw_addr =
      m_context.GetSourceReferenceAddress(source_reference);
  if (!raw_addr)
    return llvm::createStringError(llvm::inconvertibleErrorCode(),
                                   "Invalid sourceReference.");

  lldb::SBAddress source_address(*raw_addr, m_context.Target());
  ...
  lldb::SBInstructionList inst_list =
      m_context.Target().ReadInstructions(symbol.GetStartAddress(), m_line);
  ...
  m_bp = m_context.Target().BreakpointCreateBySBAddress(address);
  return llvm::Error::success();
}
...
llvm::Error SourceBreakpoint::CreateAssemblyBreakpointWithPersistenceData(
  ...
  m_bp = m_context.Target().BreakpointCreateByName(
...
void SourceBreakpoint::NotifyLogMessageError(llvm::StringRef error) {
  ...
  message += error;
  m_context.SendOutput(OutputCategory::Console, message);
}
...
bool SourceBreakpoint::BreakpointHitCallback(
  ...
  for (const SourceBreakpoint::LogMessagePart &messagePart :
    ...
    if (messagePart.is_expr) {
      ...
      output += VariableDescription(
                    value, bp->m_context.AutoVariableSummaries())
                    .display_value;
    } else {
  ...
  bp->m_context.SendOutput(OutputCategory::Console, output.c_str());
