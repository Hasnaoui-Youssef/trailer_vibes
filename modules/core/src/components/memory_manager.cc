...
// The API mutex only guards the stopped-state check here. It must not be
// held across m_strategy->Read/Write: the OpenOCD strategy blocks on
// CommandQueue::RunSync, and holding the LLDB mutex across that wait opens a
// lock-order cycle with anything that takes the API mutex from OpenOCD's
// server_loop thread (e.g. TraceManager::Status()). Each strategy takes the
// lock itself if its own calls need it.
llvm::Expected<MemoryReadResult> MemoryManager::ReadMemory(lldb::addr_t address, uint64_t count,
  ...
  const bool stopped = m_lldb_provider.WithTarget([&]() {
    return lldb::SBDebugger::StateIsStoppedState(m_lldb_provider.target.GetProcess().GetState());
  });
  if (!stopped)
...
ProcessMemoryStrategy::Write(lldb::addr_t address, llvm::ArrayRef<char> data, bool allow_partial,
  ...
  return m_lldb_provider.WithTarget([&]() -> llvm::Expected<MemoryWriteResult> {
